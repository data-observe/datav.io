...
> 关于如何开发三方插件，请参考开发者指南: https://github.com/xobserveio/xobserve/wiki#developer-guides
...
首先，我们需要下载 xObserve 源码。由于本文档只是演示如何安装插件，大家可以直接从 `main` 分支下载： https://github.com/xobserveio/xobserve ，如果是生产环境使用，请下载 release 版本。

### 下载插件

在 https://github.com/xobserveio/panel-plugin-demo 下载插件源码并解压，然后复制里面的 `demo` 文件夹到 xObserve 的源码目录下：`/DATAV_ROOT/ui/external_plugins/panel`,
...
可以看到，在 `panel` 目录下，已经有了新家的 `demo` 插件目录， 除了我们新加的 demo 目录外，还有一个 candlestick 目录，这是 xObserve 原生提供的一个外部插件，在[开发者指南中](https://github.com/xobserveio/xobserve/wiki)，我们将通过它来学习如何开发一个复杂的外部 panel 插件。
...
当看到如上提示时，说明我们的插件源码已经安装到相应的目录下，关于具体的安装到哪些目录下，可以参考[开发者指南](https://github.com/xobserveio/xobserve/wiki/Create-panel-plugin#installed-plugin-files).
