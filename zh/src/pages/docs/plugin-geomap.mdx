...
> 具体的国家编码、城市名，可以在[这里](https://github.com/observexio/observex/tree/main/ui/public/plugins/panel/geomap)查阅
...
<TextList>
    <li> 国家编码 `US`，坐标索引定义在 <Link href="https://github.com/observexio/observex/blob/main/ui/public/plugins/panel/geomap/countries.json">countries.json</Link> </li>
    <li> 城市名称 `Shanghai`, 坐标索引定义在 <Link href="https://github.com/observexio/observex/blob/main/ui/public/plugins/panel/geomap/cities.json">cities.json</Link></li>
    <li> 通过 <Code>"type":"geo"</Code>类型的字段传递的具体坐标</li>
</TextList>

ObserveX 在通过索引查询坐标时，具体的顺序是：先通过国家编码查询，如果没有找到则通过城市名称查询，如果还是没有找到则通过坐标查询。
