...
ObserveX 没有提供告警配置的功能，因为我们认为：专业的事情交给专业的人做。
...
在 ObserveX 中有两个特殊的仪表盘，它们都拥有自己特殊的 id:
...
假设你已经访问了告警页面，那么你应该会看到一堆告警。 你可能会想：为什么这里有这么多告警？我并没有在 ObserveX 中创建告警。答案很简单：这些告警都来自 TestData 数据源。

### 在 Prometheus 中添加告警规则

对于想看到告警长啥样，`TestData` 很有用，如果你想要展示真实的告警，它直等于没用。由于 Prometheus 是 ObserveX 的核心数据源，所以我们将使用它来展示如何在 ObserveX 中添加告警。
...
总之，在配置后，重新启动 Prometheus 服务器，然后回到我们的 ObserveX UI。

> 获取更多关于告警配置的信息，请访问 [Prometheus 官网](https://prometheus.io/docs/alerting/latest/overview/)。还是那句话， ObserveX 并不依赖于 Prometheus, 我们只是查询数据。 
...
可以看到，告警规则的定义与 Prometheus 中的定义完全一致，因为 ObserveX 原生支持 Prometheus 的告警规则展示，而无需任何转换。

> 如果你通过 HTTP API 加载其它系统的告警列表，那么你需要将其转换为 ObserveX 的告警格式，然后才能展示出来
