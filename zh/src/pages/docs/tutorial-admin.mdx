...
ObserveX 中的 `admin` 用户拥有所有权限，其角色是 `超级管理员`，其他用户永远无法设置为这个角色。
...
2. 点击右上角的 `新建用户` 按钮
3. 创建一个用户名和密码都是 `observex-admin` 的用户，并分配角色 `管理员 Admin`

<img src={require('@/img/docs/tutorial/add-user.jpg').default.src} alt="add user" style={{height: '300px'}}  />

4. 点击提交按钮

现在，你可以用 `observex-admin` 和 `observex-admin` 密码登录了，这个用户也可以作为 `admin` 用户来管理 ObserveX。
