...
let steps = [
  {
    ...
    body: () => (
      <>
        ...
        <p>
           然后在 `xObserveRoot/query` 目录下运行右边的命令.
        </p>
      </>
    ),
    code: {
  ...
  },
  {
    ...
    body: () => (
      <>
        <p>
          在 `xObserveRoot/query` 目录下, 运行上一步编译出的二进制可执行文件 `xobserve`。
        </p>
        <p>
    ...
    code: {
      name: 'xObserveRoot/query',
      lang: 'terminal',
      code: `./xobserve --config config.yaml
INFO[08-22|13:40:25] xObserve is listening on address            address=:10086`,
  ...
  },
  {
    ...
    body: () => (
      <p>
        打开一个新的终端窗口，进入 `xObserveRoot/ui` 目录, 然后运行右边的命令来安装依赖。
      </p>
    ),
    code: {
      name: 'DataRoot/ui',
