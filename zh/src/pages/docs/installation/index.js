  ...
  return (
    <InstallationLayout>
      ...
      <div>
        ...
        <p>如果你不修改默认监听的地址，那其实只要 `./xobserve` 就能启动服务，超级简单。现在打开浏览器访问 <Code><Link href="http://localhost:10086">http://localhost:10086</Link></Code> 看看效果吧.</p>
      </div>
      ...
      <div className="mt-8">
        ...
        <p>原因是在 xObserve server 中除了提供 api 服务外，我们同时还内置了静态文件资源服务，最最重要的是它们共享一个端口，因此对于用户来说，是完全无感知的使用。</p>
      </div>
      <div className="mt-8">
        <h2 className="text-slate-900 text-xl tracking-tight font-bold mb-3 dark:text-slate-200">
        ...
        <p className="mt-4">1. 将域名 A，例如 `https://myxobserve.io` 指向 API server 的地址，同时也是静态资源文件服务的地址</p>
        <p className="mt-4">2. 修改 `xobserve.yaml`，将字段 `override_api_server_addr_for_ui` 的值设置为 `https://myxobserve.io` </p>

        <p className="mt-4">此时，我们会从 `https://myxobserve.io` 下载静态资源，这些网页文件将访问的 API server 地址也是 `https://myxobserve.io`</p>
      ...
      <div className="mt-8">
        ...
        <p>一个很好的生产环境部署实践是将 UI 静态资源服务跟 API 服务分离部署，虽然对于 xObserve 这样的服务来说这不是必须的，但是如果你有这样的需求，可以看看下一节 <Code><Link href="/docs/installation/deploy-ui">从源码部署</Link></Code></p>
      </div>
    </InstallationLayout>
  )
}
