...
假设你在公司部署了一个 ObserveX 服务，随着时间的推移，你会有越来越多的仪表盘和用户。

在这种情况下，你可能希望使用不同的组来组织你的仪表盘，这样你和其他管理员就可以轻松地管理它们。

这种分组在 ObserveX 被称为 团队 Team。
...
如果 ops 团队使用 ObserveX，他们可能希望创建一个单独的团队来管理相应的仪表盘，因为他们可能并不关心业务相关的仪表盘长啥样。
...
让我们把 `observex-admin` 添加到 ops 团队，并给他赋予管理员角色。

> 实际上，如果一个用户在 `global` 团队中是 `管理员`，那么他也是 ObserveX 的管理员，所以我们可以得出一个结论：
...
2. 接着使用 `observex-admin` 登录并完成后续的操作步骤
...
因此，我们需要将该仪表盘转移到 `ops` 团队，但是 ObserveX 目前并没有提供任何转移操作，那只能另寻出路了。


### 仪表盘元数据

在 ObserveX 中，仪表盘的所有的设置信息都包含在一个 JSON 格式的数据中，我们称之为：`仪表盘元数据`。如果你熟悉这些设置，你可以直接在元数据中修改仪表盘设置。
