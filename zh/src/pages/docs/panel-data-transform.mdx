...
`DataFrame` 和 `SeriesData` 在格式上几乎一致，但是与 Grafana 不同的是， xObserve 并没有试图去把所有数据格式都统一为 `SeriesData`， 而是不同的图表有不同的数据格式，例如：

<TextList>
    <li>时间序列数据统一用 <Code>SeriesData</Code>  格式, 这也是 xObserve 中使用最广的，大部分图表类型都是这个数据格式，例如 Graph, Table, Stats, Gauge等.</li>
...
上面就是 `Graph` 图表需要的数据格式，而这也是 xObserve 中使用最广的 `SeriesData` 格式，与之相对应的是 Grafana 中的 DataFram，它们都是时间序列数据格式。
...
<img src="https://github.com/xobserveio/assets/blob/main/panel/panel-query.jpg?raw=true" />
