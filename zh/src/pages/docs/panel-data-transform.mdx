...
在 Grafana 中，所有的数据都是 `DataFrame` 格式， 相对应的 ObserveX 格式是 `SeriesData` 。 

`DataFrame` 和 `SeriesData` 在格式上几乎一致，但是与 Grafana 不同的是， ObserveX 并没有试图去把所有数据格式都统一为 `SeriesData`， 而是不同的图表有不同的数据格式，例如：

<TextList>
    <li>时间序列数据统一用 <Code>SeriesData</Code>  格式, 这也是 ObserveX 中使用最广的，大部分图表类型都是这个数据格式，例如 Graph, Table, Stats, Gauge等.</li>
...
对于大部分用户来说，并不需要关心数据格式。因为这些数据格式是在数据源读取到后，自动转换为图表所需的格式。但如果你想用 HTTP 数据源去某个 HTTP API 查询数据，那就要知道使用的图表所需的数据格式。毕竟外部获取的数据格式，肯定不能直接用于 ObserveX 的。
...
上面就是 `Graph` 图表需要的数据格式，而这也是 ObserveX 中使用最广的 `SeriesData` 格式，与之相对应的是 Grafana 中的 DataFram，它们都是时间序列数据格式。
...
因此，我们需要一个方法来对查询到的数据进行修改编辑，为此 ObserveX 提供了一个通用的数据转换功能。
...
最终数据格式在 ObserveX 中非常重要，无论是你要对现有的数据进行变换，还是对外部查询来的不兼容数据进行转换，最终目标都是要实现最终数据格式。
...
1. 查询到的结果格式不为 ObserveX 支持，此时需要在 `变换` 标签页中，将其转换为最终数据格式。
2. 查询到的结果格式跟最终数据格式列表中元素格式一致，此时不需要进行变换。

但如果是第二种，可以肯定的是，该接口是专门为 ObserveX 开发的数据查询接口，那么问题来了：**如果我要为 ObserveX 开发原生数据查询接口，到底应该返回什么格式？**

### 为 ObserveX 实现原生数据查询接口
