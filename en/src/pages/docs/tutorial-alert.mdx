...
You can't configure alerts rules in ObserveX, because we believe: leave professional work to professional people. 
...
ObserveX is mainly focus on Alert dispaly. There are two ways to display alerts in ObserveX:
...
There are two special dashboards predefined in ObserveX, they have their own special ids.
...
Maybe you would wonder: why there are so many alerts here? I didn't create any alerts in ObserveX. The answer is simple: the alerts you see are all from TestData datasource.
...
Because Prometheus is the core datasource of ObserveX, we will use it here to show how to add alerts to ObserveX.
...
Why we said it is no sense is because this is a `Counter` metrics, it will surely be larger than 20 in a short time after you start `Prometheus` server which will ensurely trigger the alert rules, therefore we can easily view the alerts in ObserveX.


After configuration, start Prometheus server and go back to our ObserveX ui.

> For more information, please visit [Prometheus website](https://prometheus.io/docs/alerting/latest/overview/), ObserveX does not dependent on Prometheus, we just query alerts from it, how the alerts is configured and fired, it is all the 
