...
Every user in ObserveX is assinged a role. The role determines what the user can do. 

There are currenly 3 roles in ObserveX: 
...
Among these roles, super admin is very special: only `admin` user is `Super admin` role, it's hard coded in ObserveX, 
therefor you cant chagne the username and role of `admin` user.

`Super admin` has all the permissions in ObserveX, including manage `Admin` users and teams.
...
Here is an intresting thing: in fact, there are no user roles in ObserveX, only team roles. Let's see how it works.

## ObserveX and global team

For some reasons, user roles in ObserveX are just roles in `global` team.
...
By the way, we even have not added `observex-admin` to `global` team, when we add this user and assigned `Admin` role to him, he is actually added to `global` team with the same role.

In a word, **every user you add in ObserveX is actually added to `gloal` team, when you set role for a user, you are just set his role in `global` team, and vice versa**.

## Admin role

Admin role can do many things in `ObserveX`, therefore we recommand you to manage `ObserveX` using `Admin` role user.
