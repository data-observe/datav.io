...
In grafana, all data is in `Dataframe` formats, but in ObserveX, the data passed to panel for rendering has various formats, such as 
...
<img src="https://github.com/observexio/assets/blob/main/panel/panel-query.jpg?raw=true" />
...
For this reason, we need a method to modify the data queried from the datasource , and ObserveX has provided a common data transformation function for us to do this.
...
The final data format is very important in ObserveX, no matter whether you want to transform the existing data or convert the incompatible data that query from an external HTTP API, the ultimate goal is to achieve the final data format.
...
1. The format of the result is not supported by ObserveX, in this case, you need to transform it to the final data format in the `Transform` tab.
...
But if it is the second case, one thing is certain: the HTTP API is specially developed for ObserveX, so the question is: **what format should I return if I want to develop a native data query API for ObserveX?**

### Implement native data query API for ObserveX
