...
In fact, ObserveX has a built-in datasource named `TestData`, it is used for testing purpose, and it is very useful for new users to get familiar with ObserveX.

When you select a panel visualization, ObserveX will automatically query mock data from `TestData` datasource, so you can see the chart rendering immediately.
...
If you click on a green point, the map will automatically zoom in to that point, you may think that is provide by the map, but the real answer is no, it is provided by ObserveX.

You can define various interactivity for various panels in ObserveX,  including Geomap panel. Just have a look at `Interaction` section in `Panel` tab, for more info, please refer to [Interaction](/docs/panel-interactivity)
...
Luckily, in ObserveX we can achive this by using Override feature.

### Override the column units 

In ObserveX, nearly all the panel types provides Override features, they are used for ovverride the default settings of panel for specified series or columns.
