...
Table panel is one of the most important and powerful panels in ObserveX.
...
When enabled, ObserveX will automatically assign a type related filter to each column, for example: for string type column, a text search filter will be assigned, for number type column, a number range filter will be assigned.
