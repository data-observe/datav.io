...
`admin` user in xObserve has all privileges, the role of `admin` is `Super Admin`, other users can never set to this role.
...
3. Let's create a user named `xobserve-admin` with password `xobserve-admin` and role `Admin`

<img src={require('@/img/docs/tutorial/add-user.jpg').default.src} alt="add user" style={{height: '300px'}}  />

4. Click `Submit` button

Now, you can login with `xobserve-admin` and `xobserve-admin` password. This user can also manage xobserve as `admin` user. 
