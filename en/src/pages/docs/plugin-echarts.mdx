...
In observex we have wrapped echarts options in a function, so you should feel not hard to use echarts.
...
Oh, nice, ObserveX already define a basic click event for us, let's try to click che chart and see what will happen.
...
1. Enable `Baidu map` sdk in `observex.conf` file 
...
<img src="https://github.com/observexio/assets/blob/main/plugin/shanghai-index.jpg?raw=true"/>
